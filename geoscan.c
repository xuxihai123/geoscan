...
void udpScan(int portlow, int porthigh) {
    ...
    for (scanPort = portlow; scanPort <= porthigh; scanPort++) {
        ...
        if (rx_packet(recvfd) == 1) {
            ...
            if (appl_name != NULL) printf("port %d: %s\n", scanPort, appl_name->s_name);
...
void tcpScan(int portlow, int porthigh) {
    int scanPort;
    struct servent *appl_name;
    int sendfd;
    struct sockaddr_in servaddr;
    for (scanPort = portlow; scanPort <= porthigh; scanPort++) {
        ...
        if (connect(sendfd, (struct sockaddr *) &servaddr, sizeof(servaddr)) == 0) {
            appl_name = getservbyport(htons(scanPort), protoc[0]);
...
int main(int argc, char const *argv[]) {
    ...
    if (strcmp(pro, protoc[0]) == 0) {
        ...
        printf("Usage: ./geoscan < hostname > < protocol > < portlow > < porthigh >\n");
        exit(-1);
    }
    fprintf(stderr, "Scanning host=%s, protocol=%s, ports: %d -> %d\n", host,
            pro, startPort, endPort);
    ...
    if (strcmp(pro, protoc[0]) == 0) {
        tcpScan(startPort, endPort);  // tcp scan
    } else {
